...
class MuteCog(commands.Cog):
    ...
    async def mute(self, ctx, membre: discord.Option(discord.Member), temps: discord.Option(str), raison: discord.Option(str)):
    ...
    @commands.slash_command(name="unmute", description="Retirer le mute")
    @commands.has_permissions(manage_messages=True)
    async def unmute(self, ctx, membre: discord.Option(discord.Member)):
    ...
    @commands.slash_command(name="mute_list", description="Liste des mutes actifs")
    @commands.has_permissions(manage_messages=True)
    async def mute_list(self, ctx):
        if not self.muted_users:
            await ctx.respond("📭 Aucun mute actif.", ephemeral=False)
            return
