...
class ModerationLogs(commands.Cog):
    ...
    async def on_guild_channel_create(self, channel):
        ...
        if ch:
            await ch.send(embed=log_embed("🆕 Salon créé", f"**Nom** : {channel.name}\n**Type** : {type(channel).__name__}"))
    ...
    async def on_guild_channel_delete(self, channel):
        ...
        ch = self.bot.get_channel(self.log_channel_id)
        if ch:
            await ch.send(embed=log_embed("❌ Salon supprimé", f"**Nom** : {channel.name}\n**Type** : {type(channel).__name__}"))
    ...
    async def on_member_update(self, before, after):
        ...
        if before.roles != after.roles:
            ...
            desc = ""
            if added: desc += f"**Rôles ajoutés** : {', '.join(r.mention for r in added)}\n"
            if removed: desc += f"**Rôles retirés** : {', '.join(r.mention for r in removed)}\n"
            if desc:
                await ch.send(embed=log_embed("🎭 Rôles modifiés", f"**Membre** : {after.mention}\n{desc}"))
    ...
    async def on_member_ban(self, guild, user):
        ...
        ch = self.bot.get_channel(self.log_channel_id)
        if ch:
            await ch.send(embed=log_embed("🔨 Membre banni", f"**Utilisateur** : {user} (`{user.id}`)"))
